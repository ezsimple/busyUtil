...
public class JodaUtil {
	...
	private static final DateTimeFormatter MONTH_DAY_FORMATTER = DateTimeFormat
			.forPattern("--MM-dd");

	private static final DateTimeFormatter LOCAL_DATE_FORMATTER = ISODateTimeFormat
			.date();
	...
	private static final DateTimeFormatter LOCAL_DATE_TIME_FORMATTER = new DateTimeFormatterBuilder()
			.append(LOCAL_DATE_FORMATTER).appendLiteral('T')
			.append(LOCAL_TIME_FORMATTER).toFormatter();
	...
	public static final LocalDate toLocalDate(String dateInIsoFormat) {
		...
		try {
			...
			throw new IllegalArgumentException(
					"Date must be in ISO format 'YYYY-MM-DD'.", e); //$NON-NLS-1$
		}
	}
	...
	public static final LocalTime toLocalTime(String time) {
		...
		try {
			...
			throw new IllegalArgumentException(
					"Time must be in ISO format 'hh:mm:ss'.", e); //$NON-NLS-1$
		}
	}
	...
	public static final LocalDateTime toLocalDateTime(String dateTime) {
		...
		try {
			...
			throw new IllegalArgumentException(
					"Date/time must be in ISO format 'YYYY-MM-DDThh:mm:ss'.", e); //$NON-NLS-1$
		}
	}
	...
	public static final MonthDay toMonthDay(String monthDay) {
		...
		try {
			...
			throw new IllegalArgumentException(
					"Month day must be in ISO format '--MM-dd'", e); //$NON-NLS-1$
		}
	}
	 ...
	 */
	public static final GregorianCalendar toGregorianCalendar(
			LocalDate localDate) {
		if (localDate == null) {
			return null;
		}
